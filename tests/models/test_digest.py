...
def test_digest_creation(article_summary_factory):
    ...
    summary = article_summary_factory(title="AI detects penumonia")
...
def test_digest_preserves_article_summaries(article_summary_factory):
    ...
    summary = article_summary_factory(title="Deep Learning for Cardiology")
    ...
    assert (
        digest.articles[0].ranked_article.article.title
        == "Deep Learning for Cardiology"
    )
